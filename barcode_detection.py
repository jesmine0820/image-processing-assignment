...
# Global variable to store the latest scan result
latest_scan_result = {"type": None, "data": None, "info": None}

# ----------ZXingCpp----------
def scan_barcode_generator(camera):
...
def scan_barcode_py_generator(camera):
    while True:
        frame = camera.get_frame()
        if frame is None:
            time.sleep(0.01)
...
def scan_qr_py_generator(camera):
...
def zbar_scan_qrcode_generator(camera, logo_path=None):
...
# Function to reset the scan result
